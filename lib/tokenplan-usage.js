...
export function summarize(input) {
  ...
  const series = []
  for (let i = 6; i >= 0; i--) {
    const key = localDayKey(nowMs - i * DAY_MS)
    const d = merged.days[key]
    series.push({ date: key, credits: d ? round(d.credits, 1) : 0, tokens: d ? d.tokens : 0, calls: d ? d.calls : 0 })
  }

  const todayKey = localDayKey(nowMs)
  const today = merged.days[todayKey]
  const used = windowEntries.reduce((s, e) => s + e.day.credits, 0)
