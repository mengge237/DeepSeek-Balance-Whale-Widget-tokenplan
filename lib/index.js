...
//  3) 可选：~/token-plan-tools/state.json 的 subscribed 日期当窗口锚点
...
displaySelect.className = 'dshwv-sound'
displaySelect.appendChild(soundOpt('ds', 'DeepSeek 余额'))
displaySelect.appendChild(soundOpt('qwen', '阿里 Qwen·Token Plan'))
displaySelect.appendChild(soundOpt('rotate', '两者轮换 (20s)'))
displaySelect.addEventListener('change', function () { setDisplay(displaySelect.value) })
...
rowDs.appendChild(displaySelect)
var rowTp = menuRow()
...
var display = 'ds' // 'ds' | 'qwen' | 'rotate'
var rotateTick = 0 // 轮换计数：偶数显示余额，奇数显示套餐用量
...
function showQwenAlert(p) {
  ...
  hintEl.style.display = ''
  hintEl.className = 'dshwv-hint'
  hintEl.textContent = '已用 ' + fmtCr(p.used) + '/' + fmtCr(p.cap) + ' · 剩 ' + fmtCr(p.remaining)
  hintEl.style.color = ''
  textBox.style.transition = ''
  textBox.style.opacity = ''
  bubbleBox.classList.add('dshwv-bubble-open')
...
function displayActive() {
  if (display === 'qwen') return 'qwen'
  if (display === 'rotate') return rotateTick % 2 === 0 ? 'ds' : 'qwen'
  return 'ds'
}
...
function qwenDetailLines() {
  ...
  var l2 = fmtCr(p.used) + ' / ' + fmtCr(p.cap) + ' Cr'
  return [
    { t: '第 ' + p.dayIndex + '/7 天 ' + qwenBars(p), s: 'A', c: '' },
    { t: l2, s: 'B', c: qwenColor(p) || '#536ba9' },
    { t: '今日 ' + fmtCr(p.today.credits) + ' Cr · 主力 ' + top, s: 'C', c: '' },
  ]
}
...
function renderQwen() {
  ...
  if (!p) {
    amount = shownCr !== null ? fmtCr(shownCr) : '…'
    ...
    amount = fmtCr(p.used)
    hint = 'Cr · 剩 ' + fmtCr(p.remaining) + ' · ' + fmtReset(p.resetInMs)
    color = qwenColor(p)
    if (p.quotaHitAt) hint = 'Cr · 已触顶暂停'
  }
...
fetch(SIZE_URL, { cache: 'no-store' })
  ...
  .then(function (d) {
    ...
    if (d && (d.display === 'ds' || d.display === 'qwen' || d.display === 'rotate')) {
      display = d.display
...
// 轮换模式：每 20s 在余额与套餐用量之间切一次
setInterval(function () {
  if (display !== 'rotate' || bubbleLocked()) return
  rotateTick++
  if (displayActive() === 'qwen' && !qwenData) fetchQwen(false)
  render()
}, ROTATE_MS)
...
function apply(ctx) {
    ...
    // 命中就记一笔时间戳，供面板显示「已触顶」并升级为告警（本地账本无法感知暂停）。
    function noteQuotaHit(reason) {
      try {
        const err = reason && reason.error
        if (!err || typeof err !== 'object') return
        const text = (String(err.message || '') + ' ' + String(err.code || '')).toLowerCase()
        if (!/quota|额度|exhausted|limit exceeded|too many requests/.test(text)) return
        if (/free quota/.test(text)) return // 百炼免费额度耗尽不是套餐触顶
    ...
    function handleSessionEvent(sessionId, event) {
      try {
        ...
        if (type === 'turn/end') {
          if (d.reason && d.reason.kind === 'error') noteQuotaHit(d.reason)
    ...
    function computeQwenBase() {
      ...
      // 窗口锚点：配置优先；没配置就用 token-plan-tools 的订阅日（与报表同一口径）
      let anchor = cfg.windowAnchor
      if (!anchor) {
        for (const p of TP_STATE_CANDIDATES) {
          const st = readJsonFile(p)
          if (st && typeof st.subscribed === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(st.subscribed)) {
            anchor = st.subscribed
            break
          }
        }
      }
      ...
      // 触顶信号只保留 48h：之后多半是换了套餐/重置了窗口，别再挂着旧警报
      if (led.quotaHitAt && now - led.quotaHitAt > 2 * TP.DAY_MS) {
        led.quotaHitAt = null
        markQwenDirty()
      }
